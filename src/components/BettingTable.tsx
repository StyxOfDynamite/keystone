...
const BettingTable: React.FC<BettingTableProps> = ({ onPlaceBet, bets, activeTiles, hasRolled, lastRollResult }) => {
    ...
    const renderArchSlot = (num: number) => {
        ...
        return (
            <div
                ...
                title={locked ? 'Arch bets are locked after the first roll' : undefined}
                onClick={() => !locked && isActive && onPlaceBet('ARCH', num)}
            >
                {getChip('ARCH', num)}
    ...
    const renderInsideNumberSlot = (num: number) => {
        ...
        return (
            <div
                ...
                onClick={() => isActive && onPlaceBet('INSIDE_NUMBER', num)}
            >
        ...
        <div className="betting-board">
            ...
            <div className="inside-grid">
                <div
                    ...
                    onClick={() => onPlaceBet('INSIDE_COLOR', 'RED')}
                >
                ...
                <div
                    ...
                    onClick={() => onPlaceBet('INSIDE_COLOR', 'BLACK')}
                >
                ...
                <div
                    ...
                    onClick={() => onPlaceBet('INSIDE_PARITY', 'EVEN')}
                >
                ...
                <div
                    ...
                    onClick={() => onPlaceBet('INSIDE_PARITY', 'ODD')}
                >
            ...
            <div className="betting-row">
                <div className={`bet-slot ${highlightMap.get(slotKey('INSIDE_RANGE', '4-9')) === 'won' ? 'bet-won' : highlightMap.get(slotKey('INSIDE_RANGE', '4-9')) === 'lost' ? 'bet-lost' : ''}`} style={{ width: '120px' }} onClick={() => onPlaceBet('INSIDE_RANGE', '4-9')}>
                    {getChip('INSIDE_RANGE', '4-9')}
                    <span className="slot-label">4 - 9</span>
                    <span className="slot-payout">2:1</span>
                </div>
                <div className={`bet-slot ${highlightMap.get(slotKey('INSIDE_RANGE', '10-11')) === 'won' ? 'bet-won' : highlightMap.get(slotKey('INSIDE_RANGE', '10-11')) === 'lost' ? 'bet-lost' : ''}`} style={{ width: '120px' }} onClick={() => onPlaceBet('INSIDE_RANGE', '10-11')}>
                    {getChip('INSIDE_RANGE', '10-11')}
                    <span className="slot-label">10 - 11</span>
                    <span className="slot-payout">3:1</span>
                </div>
                <div className={`bet-slot ${highlightMap.get(slotKey('INSIDE_RANGE', '12-17')) === 'won' ? 'bet-won' : highlightMap.get(slotKey('INSIDE_RANGE', '12-17')) === 'lost' ? 'bet-lost' : ''}`} style={{ width: '100px' }} onClick={() => onPlaceBet('INSIDE_RANGE', '12-17')}>
                    {getChip('INSIDE_RANGE', '12-17')}
                    <span className="slot-label">12 - 17</span>
                    <span className="slot-payout">2:1</span>
                </div>
                <div className={`bet-slot ${highlightMap.get(slotKey('INSIDE_PAIR', 'PAIR')) === 'won' ? 'bet-won' : highlightMap.get(slotKey('INSIDE_PAIR', 'PAIR')) === 'lost' ? 'bet-lost' : ''}`} style={{ width: '100px' }} onClick={() => onPlaceBet('INSIDE_PAIR', 'PAIR')}>
                    {getChip('INSIDE_PAIR', 'PAIR')}
                    <span className="slot-label">PAIRS</span>
                    <span className="slot-payout">3:1</span>
                </div>
                <div className={`bet-slot ${highlightMap.get(slotKey('INSIDE_TRIPLE', 'TRIPLE')) === 'won' ? 'bet-won' : highlightMap.get(slotKey('INSIDE_TRIPLE', 'TRIPLE')) === 'lost' ? 'bet-lost' : ''}`} style={{ width: '100px' }} onClick={() => onPlaceBet('INSIDE_TRIPLE', 'TRIPLE')}>
